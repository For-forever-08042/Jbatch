package jp.co.mcc.nttdata.batch.business.job.cmABdbldS;
...
@Slf4j
@Component
public class CmABdbldSTask extends NtBasicTask {
...
//        setenv(CmABfuncLServiceImpl.CM_MYPRGID, CM_MYPRGID);
        ...
        if (OPTION4 == null) {
            DAT_FILE = WORK_DIR + "/" + OPTION2 + ".dat";
        } else {
            DAT_FILE = WORK_DIR + "/" + OPTION4 + ".dat";
        }
        if (OPTION5 == null) {
...
//
//        if (StringUtils.isNotEmpty(shellExecuteDto01.error)) {
...
//        }
        ...
        ShellExecuteDto shellExecuteDto02 = ShellClientManager.getShellExecuteDto(CM_MYPRGID + "_02")
                ...
                .addEvn("BAD_FILE", BAD_FILE)
                .addEvn("CM_APWORK_DATE", EnvironmentConstant.CM_APWORK_DATE)
...
//                .addEvn("LOG_FILE", LOG_FILE).execute();
        ...
        //     case "2":
        //         exit_cd = Rtn_NG;
        //         APLOG_WT("copy警告終了　[" + shellExecuteDto02.result + "]", FW);
        //         SQL_CD =
        //                 Arrays.stream(LOG_FILE_DATA.replaceAll("ORA-", "\nORA-").split("\n")).filter(item -> item.startsWith("ORA-")).collect(Collectors.joining("\n"));
        //         if (StringUtils.isNotEmpty(SQL_CD)) {
        //             APLOG_WT("[SQLCODE：" + SQL_CD.split("\n")[0] + "]", FI);
        //         }
        //         break;
        //     default:
        ...
        // String SKIP_CNT = Arrays.stream(LOG_FILE_DATA.split("\n")).filter(item -> item.indexOf("スキップされた論理レコードの合計") >= 0).collect(Collectors.joining("\n"));
...
//                    .addEvn("LOG_FILE", BAD_FILE).execute();
            IconvUtil.main(SystemConstant.Shift_JIS, SystemConstant.UTF8, BAD_FILE, BAD_FILE + "_utf8");
            return Rtn_NG;
        }
