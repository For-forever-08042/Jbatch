package jp.co.mcc.nttdata.batch.business.job.cmBTfspoS;
...
/**
 #-----------------------------------------------------------------------------------
 #    名称          ：  不正防止データ(1回のPOS加算で1万P以上付与)ファイル作成シェル
 #    プログラムID  ：  cmBTfspoS
 #
 #    【処理概要】
 #       HSポイント日別情報YYYYMMから不正防止データ(1回のPOS加算で1万P以上付与)
 #       を作成する。
 #
 #    【引数説明】
 #       なし
...
@Slf4j
@Component
public class CmBTfspoSTask extends NtBasicTask {
    ...
    public int taskExecuteCustom(String[] args) {
        ...
        String FILE_NAME = "FUSEI_POS_POINT_OVER_";
        //#  システム日付
        //SYS_YYYYMMDD=`date -d "$(date +%Y%m01) -1 days " +%Y%m%d`
        String SYS_YYYYMMDD = DateUtil.getLMLast_YYYYMMDD();
        ...
        if (shellExecuteDto.RTN0()) {
            ...
            cmABaplwB.main(getExecuteBaseParam().P(CM_MYPRGID).M("不正防止データ(1回のPOS加算で1万P以上付与）取得に失敗しました。SQLCODE=" + SQL_CD).FW());
