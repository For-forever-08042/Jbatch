package jp.co.mcc.nttdata.batch.business.com.cmABgprmB;
...
/******************************************************************************/
/*   プログラム名	： パラメータファイル名取得処理（cmABgprm.c）         */
/*                                                                            */
/*   【処理概要】                                                             */
...
/******************************************************************************/
@Service
public class CmABgprmBServiceImpl extends CmABfuncLServiceImpl implements CmABgprmBService{

    boolean DBG_getParamModName = true;                           /* getParamModName    */
...
/******************************************************************************/
    ...
    /*                                                                            */
    /*	書式                                                                  */
    /*	static int  getParamModName(char *pname char *ppath)                  */
    /*                                                                            */
    /*	【説明】                                                              */
    ...
    /******************************************************************************/
    @Override
    public int getParamModName(StringDto pname, String dirname, StringDto pname2) {
        LinkedList<File> d;
        File dd;
        int ver1;
        int ver2;
        String logbuf = null;
        String wk_pname = null;

        if (DBG_getParamModName) {
            C_DbgMsg("cmABgprmB : getParamModName: %s\n", "start");
            C_DbgMsg("cmABgprmB : getParamModName: pname=[%s]\n", pname);
        }

        strcpy(pname2, "");

        if (DBG_getParamModName) {
            C_DbgMsg("cmABgprmB : getParamModName: opendir(%s) start\n", dirname);
        }
        ...
        if (d == null) {
            ...
            if (DBG_getParamModName) {
                C_DbgMsg("cmABgprmB : getParamModName: opendir(%s) = null\n", dirname);
            }

            logbuf = sprintf(logbuf, "opendir(%s)", dirname);
            /* ＡＰログを出力(903) */
        ...
        if (DBG_getParamModName) {
            C_DbgMsg("cmABgprmB : getParamModName: opendir(%s) end\n", dirname);
        }
        ...
        for (; ; ) {
            ...
            if (DBG_getParamModName) {
                C_DbgMsg("cmABgprmB : getParamModName: dd.getName() = %s\n", dd.getName());
            }
            ...
            if (strlen(wk_pname) == 0) {
                ...
                if (DBG_getParamModName) {
                    C_DbgMsg("cmABgprmB : getParamModName: wk_pname = %s\n", wk_pname);
                }
            } else {
                /* ２番目以降           */
                /* バージョンを比較する */
                ver1 = atoi(wk_pname.substring(9));
                ver2 = atoi(dd.getName().substring(9));
...
/******************************************************************************/
    ...
    public MainResultDto main(int argc, String[] argv) {
        ...
        StringDto prm_name = new StringDto(); /* 起動モジュール名 */
        int loop_ix;
        StringDto exec_path = new StringDto(); /* 該当ファイル名 フルパスでバージョン番号付き  */
        int ret;
        String penv;
        ...
        if (DBG_main) {
            C_DbgMsg("cmABgprmB : getExecModName %s\n", "start");
        }

        penv = getenv("CM_APPARAM");
        if (StringUtils.isEmpty(penv)) {
            /* 環境変数取得失敗 */

            if (DBG_getParamModName) {

                C_DbgMsg("cmABgprmB : getenv(%s)=null\n", "CM_APPARAM");
            }

            msg_param = sprintf(msg_param, "getenv(%s)", "CM_APPARAM");
            /* ＡＰログを出力(903) */
        ...
        ret = getParamModName(prm_name, param_path, exec_path);

        if (DBG_main) {
            C_DbgMsg("cmABgprmB : getParamModName=%d\n", ret);
        }

        if (ret != C_const_OK) {
            /* パラメータファイル名を取得できない */
            msg_param = sprintf(msg_param, "入力パラメータファイル名の取得異常(%s)", prm_name);
            /* ＡＰログを出力(911) */
        ...
        C_DbgEnd("cmABgprmB", ret, rtn_status, 0);
