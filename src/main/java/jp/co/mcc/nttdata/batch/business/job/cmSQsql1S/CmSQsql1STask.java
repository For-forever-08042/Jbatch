package jp.co.mcc.nttdata.batch.business.job.cmSQsql1S;
...
#-------------------------------------------------------------------------------
    ...
    #      2.00 :   2014/09/26 SSI.吉川：帳票ファイル名修正
    #-------------------------------------------------------------------------------
    #  $Id:$
    #-------------------------------------------------------------------------------
 ...
 **/
@Component
public class CmSQsql1STask extends NtBasicTask {
  ...
  public int taskExecuteCustom(String[] args) {
    ...
    if (shellExecuteDto.RTN0()) {
      //
      ArrayList<String> SQL_CD_data=  FileUtil.findByRegex(CM_APWORK_DATE,"^count_card_month.{8}.utf");
      SQL_CD_data.forEach(item->{
        SQL_CD+= FileUtil.SQL_CD_ORA_FILE(item);
      });
      APLOG_WT("会員入会状況の集計処理に失敗しました。SQLCODE={" + SQL_CD + "}", FW);
      cmABaplwBServiceImpl.main(getExecuteBaseParam().P(CM_MYPRGID).E(CM_MYPRGNAME).FW());
      return Rtn_NG;
    }

    FileUtil.deleteFileByRegx(CM_APWORK_DATE ,"^count_card_month.{8}" + ".csv");
    ...
    //#  保持期間を超過したファイルを削除
    //###########################################
    int FILE_CNT = FileUtil.countLinesByRegex(CM_PCRENKEI_KOJINNASHI, "^count_card_month.{8}" + ".csv");
    if (FILE_CNT >= 3){
      List<String> TEMP_FILE1_DATE = FileUtil.findByRegex(CM_PCRENKEI_KOJINNASHI ,
          "^count_card_month.{8}" + ".csv");
