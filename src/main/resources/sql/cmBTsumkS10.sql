...
WHERE 
 ...
 AND D.システム年月日 < CAST(TO_CHAR(LAST_DAY(ADD_MONTHS(TO_DATE(CAST(:SQL_DATE AS TEXT)), -1))+4,'YYYYMMDD') AS NUMERIC)          -- システム年月日 < 当月/4
 AND D.登録年月日 <= CAST(TO_CHAR(LAST_DAY(ADD_MONTHS(TO_DATE(CAST(:SQL_DATE AS TEXT)), -1)),'YYYYMMDD') AS NUMERIC)                -- 前月/末
GROUP BY
              D.システム年月日
             ,D.登録年月日
             ,D.会社コードＭＣＣ
