...
SELECT
    ...
    NVL((CASE WHEN TBLA.データ区分 = '1'
              THEN TO_CHAR(TBLD.旧企業コード, 'FM0000')
              ELSE TO_CHAR(TBLF.新会社コード, 'FM0000')
          END), RPAD(' ', 4)) ,',',   
    NVL((CASE WHEN TBLA.データ区分 = '1'
             THEN TO_CHAR(CAST(TBLA.店舗コード AS NUMERIC), 'FM0000')
             ELSE TO_CHAR(TBLF.新店番号, 'FM0000')
         END), RPAD(' ', 4))  ,',',
...
FROM
      ...
      LEFT JOIN PS店表示情報ＭＣＣ TBLD ON TBLA.会社コード  = CAST(TBLD.会社コード AS TEXT)  AND TBLA.店舗コード = CAST(TBLD.店番号 AS TEXT) AND :SBATDATE >= TBLD.開始年月日 AND :SBATDATE <= TBLD.終了年月日
      LEFT JOIN PS店舗変換マスタ TBLF ON 2500 = TBLF.旧会社コード AND TBLA.店舗コード = CAST(TBLF.旧店番号 AS TEXT)
WHERE
-- 2023/06/09 MCCMPH2 ADD START
 (( TBLA.データ区分 = '1' AND ((TBLA.部門コード IS NULL OR TBLA.部門コード='') OR TBLA.部門コード <> '9999999' ))
       OR  TBLA.データ区分 = '2' )
-- 2023/06/09 MCCMPH2 ADD END
;
